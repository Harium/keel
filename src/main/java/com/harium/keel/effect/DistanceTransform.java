...
public class DistanceTransform {
     ...
     * @return Distance map.
     */
    public float[][] Compute(ImageSource fastBitmap) {
        int width = fastBitmap.getWidth();
        int height = fastBitmap.getHeight();

        // Handle grayscale or rgb
        int[] bPixels = EffectHelper.getGrayData(fastBitmap);
     ...
     * @return FastBitmap.
     */
    public ImageSource toImageSource(boolean grayscale) {

        int width = image[0].length;
        int height = image.length;

        if (grayscale) {
            OneBandSource source = new OneBandSource(width, height);

            for (int i = 0; i < height; i++) {
                for (int j = 0; j < width; j++) {
                    int gray = (int) Tools.Scale(0, max, 0, 255, image[i][j]);
                    source.setRGB(j, i, gray);
                }
            }
            return source;
        } else {
            MatrixSource source = new MatrixSource(width, height);

            for (int i = 0; i < height; i++) {
                for (int j = 0; j < width; j++) {
                    int gray = (int) Tools.Scale(0, max, 0, 255, image[i][j]);
                    int rgb = ColorHelper.getRGB(gray, gray, gray);
