...
public class GridIndex
{
	 ...
	 */
	public GridIndex( DelaunayTriangulation delaunay, int xCellCount,
	...
	public void updateIndex(Iterator<Triangle> updatedTriangles) {
		...
		if(!indexRegion.contains(updatedRegion)) {
			...
			PointInt2D minInvalidCell = getCellOf(updatedRegion.getMinPoint());
			PointInt2D maxInvalidCell = getCellOf(updatedRegion.getMaxPoint());
	 ...
	 */
	private Triangle findValidTriangle(PointInt2D minInvalidCell)
	{
		...
		if(minInvalidCell.getX() == 0 && minInvalidCell.getY() == 0)
