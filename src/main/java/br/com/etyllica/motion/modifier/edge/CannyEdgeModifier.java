...
/**
 * Forked from http://www.tomgibara.com/computer-vision/CannyEdgeDetector.java
 * Originally made by Tom Gibara
 *
 */
public class CannyEdgeModifier implements EdgeModifier {
	...
	public List<Point2D> modify(ImageSource source, Component component) {
		...
		initArrays();
		readLuminance(source, component);
	...
	private List<Point2D> output(int[] data) {
		List<Point2D> points = new ArrayList<Point2D>();
		
		int invalid = -16777216;
		
		for (int i = 0; i < picsize; i++) {
			if (data[i] != invalid) {
				points.add(new Point2D(i%width, i/width));	
			}
		}
	...
	private void readLuminance(ImageSource source, Component component) {
		...
		for (int i = component.getY(); i < height; i++) {
			for (int j = component.getX(); j < width; j++) {
				int c = j + width * i;
				pixels[c] = source.getRGB(j, i);
				int p = pixels[c];
				int r = (p & 0xff0000) >> 16;
			int g = (p & 0xff00) >> 8;
			int b = p & 0xff;
			data[c] = luminance(r, g, b);
			}
		}
	}
